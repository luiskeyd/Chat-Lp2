...
public class Servidor {
    ...
    public static Set<Usuario> usuarios = new HashSet<>(); // Conjunto de usuários conectados ao servidor
    ...
    public static void main (String[] args){
        try (ServerSocket servidor = new ServerSocket(porta)) { // Criação do servidor
            ...
            while (true){ // Esperar uma conexão de algum usuário
                ...
                // Cria uma nova thread para lidar com cada cliente conectado ao servidor paralelamente
                Thread t = new Thread(new ClienteConectado(cliente));
                t.start(); // Inicia a thread
            }
        } catch (Exception e) { // Caso não consiga criar o servidor
